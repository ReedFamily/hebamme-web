...
const editFaq = function (lnk) {
  ...
  $.get(url, function (res) {
    if (res.status == 200) {
      ...
      faqEditor.value(res.faq.message);
      $("#edit-faq-dialog").modal("show");
    }
  });
};
...
const editFaqEvent = function (event) {
  ...
  editedFaqData.message = faqEditor.value(); //$("#faqMessage").val();
...
const newFaqEvent = function (event) {
  ...
  newFaqData.message = faqEditor.value(); //$("#faqMessage").val();
